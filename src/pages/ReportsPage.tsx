...
export function ReportsPage() {
  ...
  const byClient = useMemo(() => {
    const map = new Map<
      string,
      { name: string; n: number; revenue: number; margin: number; vat: number; cost: number; km: number }
    >()
    for (const o of live) {
      ...
      cur.cost += calcTripCost(o).totalKop
  ...
  return (
    <div className="space-y-4 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card className="p-5">
          <div className="text-[11px] uppercase tracking-[0.14em] text-[#6d614c]">Себестоимость рейсов</div>
          <div className="font-serif text-3xl">{formatMoney(tripCost)}</div>
          <div className="text-xs text-[#6d614c]">ЗП водителя + Платон + топливо 35 л/100 км</div>
        </Card>
      </div>
